export const boundFunc = (pos, scale, {
    width,
    height,
}) => {
...
export const dragBoundFunc = (layer) => (pos) => {
    if (!layer) {
        return null;
    }
...
export const convertCoordsFromImageScale = (image = {}, view = {}, coords = {}) => {
    const coord = {};
    coord.x = coords.x / (image.width / view.width);
    coord.y = coords.y / (image.height / view.height);
    return coord;
};

export const convertCoordsToImageScale = (image = {}, view = {}, coords = {}) => {
    const coord = {};
    coord.x = coords.x * (image.width / view.width);
    coord.y = coords.y * (image.height / view.height);
    return coord;
